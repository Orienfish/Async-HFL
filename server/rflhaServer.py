...
class RflhaServer(SyncServer):
    ...
    def run(self, logger=None):
        ...
        for gateway in self.gateways:
            ...
            # sync gateway model
            gateway.rflha_gateway_configure(self.config, 0.0)

            # Run gateway sync and get the new time
        ...
        for round in range(1, rounds + 1):
            ...
            print(next_gw_agg)
            logging.info('Select gateway {}, time {} s'.format(select_gateway.gateway_id, T_cur))
            ...
            # Configure the gateway: load async global model and save as
            # sync gateway model
            select_gateway.rflha_gateway_configure(self.config, T_cur)
    ...
    def federated_async(self, reports, staleness):
        ...
        logging.info('{} alpha: {} staleness: {} alpha_t: {}'.format(
            self.staleness_func, self.alpha, staleness, alpha_t
        ))
